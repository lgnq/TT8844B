...
code unsigned char tw884x_ntsc[] = 
{
    // init with NTSC
	...
	0xFF, 0x00, // Page 0
	0x40, 0x00,
	//0x41, 0x30,				// enable input H cropping
	0x41, 0x20,				// disable input H cropping
	0x42, 0x00,
	0x43, 0x7B,
	0x47, 0x10,
	0x4A, 0x03,			// VS Delay
...
code unsigned char tw884x_evb_init[] = 
{
#if 0
    0xFF, 0x00,  //; Page 0
    0x00, 0x44,
...
void i2c_write(unsigned char addr, unsigned char idx, unsigned int val)
{
    ...
    //发送地址和数据读写方向  
    //低位为0，表示写数据  
    i2c_write_byte(addr | 0);  
    
    //写入索引  
    i2c_write_byte(idx);  

    //写入数据  
    i2c_write_byte(val>>8);
...
void backlight_onoff(unsigned char onoff)
{
    if (onoff)
        FPBIAS = 0;
    else
...
void LGWidePanelGammaOn(void)
{
//	FPPWC = 1;						// OFF
	i2c_write(0xE8, 0x00, 0x0040);
	i2c_write(0xE8, 0x02, 0x03DA);
	i2c_write(0xE8, 0x04, 0x03D3);
	i2c_write(0xE8, 0x06, 0x0366);
	i2c_write(0xE8, 0x08, 0x0335);
	i2c_write(0xE8, 0x0A, 0x0300);
	i2c_write(0xE8, 0x0C, 0x02cd);
	i2c_write(0xE8, 0x0E, 0x029b);
	i2c_write(0xE8, 0x10, 0x0241);
	i2c_write(0xE8, 0x12, 0x0218);
	i2c_write(0xE8, 0x14, 0x01fb);
	i2c_write(0xE8, 0x16, 0x01cc);
	i2c_write(0xE8, 0x18, 0x0173);
	i2c_write(0xE8, 0x1A, 0x013f);
	i2c_write(0xE8, 0x1C, 0x010f);
	i2c_write(0xE8, 0x1E, 0x00d8);
	i2c_write(0xE8, 0x20, 0x00a9);
	i2c_write(0xE8, 0x22, 0x003e);
	i2c_write(0xE8, 0x24, 0x0034);
//	FPPWC = 0;						// ON
}
...
void tw884x_init(void)
{
    ...
    if (id == 0x44)
    {
        ...
        //device_initialize(tw884x_ntsc);

        device_initialize(tw884x_evb_init);
        ...
        fosd_display_string(0, "hello", 0x08);
